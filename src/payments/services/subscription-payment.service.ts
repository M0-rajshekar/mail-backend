...
export class SubscriptionPaymentService {
    ...
    async processPayment(
        ...
        const isSolanaRequest =
            requestedNetwork &&
            requestedNetwork.toLowerCase().includes('solana');
        ...
        if (!middleware) {
            ...
            res.status(500).json({
                error: 'Payment middleware not initialized',
            });
            return;
        }

        this.logger.log(
            `Applying X402 middleware for ${plan} ${billingPeriod}`,
        );
    ...
    private async processOfficialSolanaX402V2(
        ...
        try {
            this.logger.log(
                `🚀 X402 v1 processing for ${plan} ${billingPeriod}`,
            );
            ...
            if (!xPaymentHeader) {
                    ...
                    accepts: [
                        {
                            ...
                            asset: this.configService.get(
                                'NEXT_PUBLIC_USDC_MINT_ADDRESS',
                            ),
                            payTo: this.configService.get(
                ...
                if (!paymentRequirements.accepts[0].asset) {
                    throw new Error(
                        'NEXT_PUBLIC_USDC_MINT_ADDRESS environment variable is required',
                    );
                }
            ...
            try {
                ...
                this.logger.error(
                    '❌ Failed to parse X-PAYMENT header:',
                    parseError,
                );
                res.status(402).json({
            ...
            try {
                ...
                this.logger.log(
                    '🔐 Calling v1 facilitator for payment verification...',
                );

                // Use PayAI v1 facilitator endpoints - no hardcoded URLs
                const facilitatorUrl = this.configService.get(
                    'X402_FACILITATOR_URL',
                );
                if (!facilitatorUrl) {
                    throw new Error(
                        'X402_FACILITATOR_URL environment variable is required',
                    );
                }
                ...
                try {
                    ...
                    if (supportedResponse.ok) {
                        ...
                        this.logger.log(
                            ...
                            JSON.stringify(
                                supportedData.kinds.filter((k: any) =>
                                    k.network.includes('solana'),
                                ),
                                null,
                                2,
                            ),
                        ...
                        this.logger.warn(
                            'Could not fetch supported networks:',
                            supportedResponse.status,
                            supportedResponse.statusText,
                        );
                    }
                } catch (supportError) {
                    this.logger.warn(
                ...
                if (!settleResult.success) {
                    this.logger.error(
                        '❌ Payment settlement unsuccessful:',
                        settleResult,
                    );
                    throw new Error(
                        `Payment settlement failed: ${settleResult.error || 'Unknown error'}`,
                    );
                }

                this.logger.log(
                    '✅ v1 facilitator settled payment (real USDC transfer)',
                );
                this.logger.log(
                    '📄 Transaction hash:',
                ...
                if (!settleResult.transaction) {
                    throw new Error(
                        'Settlement completed but no transaction hash received',
                    );
                }
                ...
                if (!walletAddress) {
                    ...
                    try {
                        ...
                        const signers = transaction.signatures.filter(
                            (sig) => sig.signature !== null,
                        );
                        if (signers.length > 0) {
                            walletAddress = signers[0].publicKey.toString();
                            this.logger.log(
                                `✅ Extracted wallet address from transaction signers: ${walletAddress}`,
                            );
                        } else {
                            throw new Error(
                                'No valid signers found in transaction',
                            );
                        }
                    } catch (extractError) {
                        this.logger.error(
                            '❌ Could not extract wallet address from payment:',
                            extractError,
                        );
                        throw new Error(
                            'Payment processed but could not identify payer wallet address',
                        );
                    }
                }
                ...
                if (!this.validateWalletAddress(walletAddress)) {
                    throw new Error(
                        `Invalid wallet address extracted from payment: ${walletAddress}`,
                    );
                }

                this.logger.log(
                    `✅ X402 v1 payment verified and settled for wallet: ${walletAddress}`,
                );
            } catch (facilitatorError) {
                this.logger.error(
                    '❌ v1 facilitator error:',
                ...
                if (
                    ...
                    facilitatorError.message.includes(
            ...
            try {
                ...
                this.logger.log(
                    `🎉 X402 v1 payment processed successfully:`,
                    result,
                );
                res.json(result);
            ...
            res.status(500).json({
                ...
                message:
                    error.message ||
                    'An error occurred while processing X402 v1 payment',
            });
        }
    }
    ...
    private extractWalletAddress(req: Request): string {
        ...
        if (!walletAddress && xPaymentHeader) {
            try {
                ...
                this.logger.log(
                    `Extracted wallet from X-PAYMENT: ${walletAddress}`,
                );
            } catch (error) {
    ...
    private extractNetworkInfo(req: Request): string {
        ...
        if (
            ...
            const subscriptionPlans = Object.values(SubscriptionTier).map(
                (tier) => tier.toLowerCase(),
            );
            if (
                networkFromPath &&
                !subscriptionPlans.includes(networkFromPath)
            ) {
