...
import { 
  getX402ExpressSupportedNetworks
...
export class X402MiddlewareService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.logger.log(`X402MiddlewareService initialized - middleware available: ${this.isAvailable()}`);
  }
  ...
  private initializeMiddleware(): void {
    ...
    try {
      ...
      if (!paymentMiddleware) {
        throw new Error('paymentMiddleware function not found in x402-express package');
      }
      ...
      this.logger.log('Creating X402 middleware with routes:', Object.keys(routes));
      ...
      this.logger.log('✅ X402 middleware initialized successfully with PayAI facilitator');
      const supportedNetworks = getX402ExpressSupportedNetworks().map(n => n.name);
      this.logger.log(`EVM networks supported by x402-express: ${supportedNetworks.join(', ')}`);
      this.logger.log('Solana networks will be handled with custom X402 implementation');
    } catch (error) {
      this.logger.error('❌ Failed to initialize X402 middleware:', error.message);
  ...
  private generateAllNetworkRoutes(pricing: any): Record<string, X402RouteConfig> {
    const routes: Record<string, X402RouteConfig> = {};
    const maxTimeoutSeconds = this.configService.get('X402_MAX_TIMEOUT_SECONDS', 300);

    // Get subscription plans and billing periods from Prisma enums
    const plans = Object.values(SubscriptionTier).filter(tier => tier !== SubscriptionTier.FREE); // Exclude FREE tier from X402 payments
    const periods = Object.values(BillingPeriod);
    const networks = getX402ExpressSupportedNetworks().map(network => network.name); // Dynamic EVM networks for x402-express
    ...
    for (const plan of plans) {
      for (const period of periods) {
        ...
        if (!price) {
          this.logger.error(`Missing price configuration for ${plan} ${period}`);
          continue;
        }
    ...
    this.logger.log('Generated X402 routes for EVM networks only:', Object.keys(routes));
    this.logger.log('Solana networks will be handled with custom X402 implementation');
    return routes;
  }
  ...
  private getFacilitatorUrl(): string {
    return this.configService.get('X402_FACILITATOR_URL') || 
  ...
  getEvmPaymentAddress(): string {
    return this.configService.get('EVM_PAYMENT_ADDRESS') || 
  ...
  private getSubscriptionPricing(): any {
    ...
    Object.values(SubscriptionTier).forEach(tier => {
      ...
      Object.values(BillingPeriod).forEach(period => {
        ...
        if (envPrice) {
          ...
          if (planConfig) {
            pricing[tier][period] = period === BillingPeriod.YEARLY 
    ...
    this.logger.log('X402 Subscription Pricing Configuration:', JSON.stringify(pricing, null, 2));
