...
export class AttachmentStorageService {
  ...
  constructor(private readonly configService: ConfigService) {
    this.bucket = this.configService.get<string>('S3_BUCKET') || 'agentmail-attachments';
  ...
  async storeAttachment(
    ...
    const sanitizedFilename = attachment.filename.replace(/[/\\:*?"<>|\x00-\x1f]/g, '_');
    ...
    try {
      ...
      this.logger.log(`Stored attachment: ${key} (${attachment.size} bytes)`);
  ...
  async storeAttachments(
    ...
    for (const attachment of attachments) {
      ...
      const storedAttachment = await this.storeAttachment(emailId, id, attachment);
