...
export class CloudflareEmailService {
  ...
  async sendEmail(params: SendEmailParams): Promise<{ messageId: string }> {
    ...
    if (!apiToken || !accountId) {
      this.logger.warn('Cloudflare credentials not configured, using mock sender');
    ...
    try {
      ...
      this.logger.error('Failed to send email via Cloudflare:', error.message);
   ...
   */
  async sendEmailSmtp(params: SendEmailParams): Promise<{ messageId: string }> {
    // Fallback SMTP implementation could go here
    // For now, just log and return mock
    this.logger.log(`SMTP fallback for email to: ${Array.isArray(params.to) ? params.to.join(', ') : params.to}`);
